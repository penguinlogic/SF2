...
% function [bits, huffval] = huffdes(huffhist)
% Generate the JPEG tables bits and huffval from the 256-point
% histogram of values huffhist.
% This is based on the algorithms in the JPEG Book Appendix K.2.

scale = (127*256) / sum(huffhist);

% Scale huffhist to sum to just less than 32K and reshape to be square.
freq = reshape(huffhist*scale,16,16);
...
freq(2:11,:) = freq(2:11,:) + 1;  
freq(1,[1 16]) = freq(1,[1 16]) + 1;
